...
export class AuthGoogleController {
  ...
  redirectToGoogle(@Session() session: any) {
    ...
    const url = this.oAuthService.generateRedirectUrl(OAUTH_PROVIDERS.GOOGLE, state);
    session.state = state;
  ...
  async handleOAuthCallback(@Query() { code, state }: HandleOAuthCallback, @Session() session: any) {
    ...
    const userInfo = await this.oAuthService.handleOAuth2Callback(OAUTH_PROVIDERS.GOOGLE, code);
    const jwt = await this.authService.generateJwt(userInfo.id, OAUTH_PROVIDERS.GOOGLE);
    return { jwt };
  }
}
