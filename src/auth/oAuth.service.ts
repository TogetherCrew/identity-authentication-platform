...
export class OAuthService {
  ...
  generateRedirectUrl(provider: string, state: string): string {
    const params = {
      ...
      redirect_uri: this.configService.get<string>(`${provider}.redirectUri`),
      response_type: 'code',
  ...
  async exchangeCodeForToken(provider: string, code: string): Promise<{ access_token: string }> {
    const clientId = this.configService.get<string>(`${provider}.clientId`);
    const clientSecret = this.configService.get<string>(`${provider}.clientSecret`);
    const redirectUri = this.configService.get<string>(`${provider}.redirectUri`);
    ...
    try {
      ...
      this.logger.error(`Failed to exchange ${provider} code for token`, { error });
      throw new HttpException(`Failed to exchange ${provider} code for token`, HttpStatus.BAD_REQUEST);
    }
  }
  ...
  async getUserInfo(provider: string, accessToken: string): Promise<any> {
    ...
    try {
      ...
      this.logger.error(`Failed to retrieve user information from ${provider}`, { error });
      throw new HttpException(`Failed to retrieve user information from ${provider}`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async handleOAuth2Callback(provider: string, code: string): Promise<any> {
    try {
      ...
      this.logger.error(`Error handling ${provider} OAuth2 callback`, error);
      throw new HttpException(`Failed to handle ${provider} OAuth2 callback`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
}
